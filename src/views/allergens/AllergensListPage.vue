...
onMounted(async () => {
  ...
  try {
    const response = await useFetchAPI({
      url: '/facts/category/3', // Assuming '/facts/facts' is your endpoint
      method: 'GET',
      // No data needed for a GET request (unless you're adding query parameters)
    });
    allergens.value = response.data.facts;
  } catch (error) {
    ...
    <ion-content class="ion-padding" :fullscreen="true">
      <div class="flex items-center mb-2">
        ...
        <h5 class="font-bold ms-2 mt-1 ">
          Allergens in AllerTrac!
        </h5>
      </div>

      <img class="w-[130px] my-5 mx-auto" src="/images/allergies/allergens-logo.png" alt="Food" />

      <!-- Display list of facts -->
      <ul v-if="!isLoading && !error">
        <li v-for="(allergen, index) in allergens" :key="index">
          <router-link :to="{ name: allergen.path, params: { id: allergen.id } }" class="font-bold hover:underline">
            <button
                ...
                :style="{ backgroundImage: `url(${allergen.cover_image}`}"
            >
              <h6 class="text-sm text-[#F4EDEF]" style="text-shadow: 4px 4px 6px rgba(0, 0, 0, 1);">{{ allergen.description }}</h6>
              <h3 class="font-bold text-lg text-[#F4EDEF]" style="text-shadow: 4px 4px 6px rgba(0, 0, 0, 0.7);">{{ allergen.title }}</h3>
            </button>
          </router-link>
        </li>
      </ul>
